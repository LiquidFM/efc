...
bool UdpReceiver::receive(void *buffer, size_t size, size_t &received)
{
	...
	{
    	ssize_t res = ::recvfrom(descriptor(), buffer, size, 0, NULL, NULL);
...
bool UdpReceiver::receive(void *buffer, size_t size, size_t &received, String &peerName, in_port_t &peerPort)
{
    if (LIKELY(waitForRead() == true))
	{
    	...
    	ssize_t res = ::recvfrom(descriptor(), buffer, size, 0, reinterpret_cast<struct sockaddr *>(&addr), &addrlen);
